...
class product_spidersPipeline(object):
    ...
    def spider_closed(self, spider):
        ...
        #         exporter.finish_exporting()
        elif spider.name == 'amazon_spider':
            dir_path = 'output'.format(spider.name)
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)

            filepath = '{}/amazon_spider_result.xlsx'.format(dir_path)
            if os.path.isfile(filepath):
                os.remove(filepath)
            workbook = xlsxwriter.Workbook(filepath, {'strings_to_urls': False})
            sheetWorks = workbook.add_worksheet('amazon)')
            ...
            for col, val in enumerate(EXPORT_FIELDS):
                sheetWorks.write(0, col, val)

            for row, data in enumerate(spider.result_data_list):

                for col, field in enumerate(EXPORT_FIELDS):
                    if data[field]:
                        sheetWorks.write(row + 1, col, data[field].encode('utf-8'))
